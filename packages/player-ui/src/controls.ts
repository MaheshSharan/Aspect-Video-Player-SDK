...
/**
 * Subtitle button component (toggles subtitle selector or indicates no subtitles).
 */
export class SubtitleButton implements UIComponent {
    readonly name = 'subtitle-button';
    ...
    render(): HTMLElement {
        ...
        this.element = document.createElement('button');
        this.element.className = `${prefix}${CSS_CLASSES.BUTTON} ${prefix}${CSS_CLASSES.BUTTON_SUBTITLES}`;
        this.element.setAttribute('aria-label', 'Subtitles');
        this.element.innerHTML = this.getIcon();
        this.element.addEventListener('click', this.handleClick);
    ...
    update(_snapshot: PlayerSnapshot): void {
        // Could update to show active subtitle track
    }
